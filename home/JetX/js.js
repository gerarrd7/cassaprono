class JetXPredictor {
    ...
    onSignalClick() {
        ...
        setTimeout(function() {
            ...
            GameStateManager.save('jetx', { coefficient: signal, countdownEnd: Date.now() + self.COOLDOWN * 1000, playAt: playAt });
    ...
    restoreState() {
        var saved = GameStateManager.load('jetx');
        if (!saved) return;
        var t = this.t();
        var now = Date.now();
...
document.addEventListener('DOMContentLoaded', function() { new JetXPredictor(); });