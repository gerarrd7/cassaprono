class OverheatPredictor {
    ...
    initBubbles() {
        ...
        for (var i = 0; i < 6; i++) {
            ...
            var x    = (Math.random() * 92 + 2).toFixed(1) + '%';
            var sz   = (Math.random() * 12 + 5).toFixed(0) + 'px';
            var dur  = (Math.random() * 6 + 5).toFixed(1) + 's';
            var del  = (Math.random() * 6).toFixed(1) + 's';
            bub.style.cssText = '--x:' + x + ';--sz:' + sz + ';--dur:' + dur + ';--delay:' + del;
    ...
    onSignalClick() {
        ...
        setTimeout(function() {
            ...
            GameStateManager.save('overheat', { coefficient: signal, countdownEnd: Date.now() + self.COOLDOWN * 1000 });
            self.startCooldown(self.COOLDOWN);
        }, 5000);
    }

    restoreState() {
        var saved = GameStateManager.load('overheat');
        if (!saved) return;
        var t = this.t();
        var now = Date.now();
...
document.addEventListener('DOMContentLoaded', function() { new OverheatPredictor(); });
