class AviatorPredictor {
    ...
    onSignalClick() {
        ...
        setTimeout(function() {
            ...
            GameStateManager.save('aviator', { coefficient: signal, countdownEnd: Date.now() + self.COOLDOWN * 1000, playAt: playAt });
    ...
    restoreState() {
        var saved = GameStateManager.load('aviator');
        if (!saved) return;
        var t = this.t();
        var now = Date.now();
...
document.addEventListener('DOMContentLoaded', function() { new AviatorPredictor(); });
